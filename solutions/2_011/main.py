"""Inference entrypoint for the DaT Parkinson's Challenge - model 011.
...
008a with the sm0 family retrained under an added spatially-correlated
count-noise augmentation. Two independent training draws ship pooled (30 checkpoints):
keeping only the luckier draw would be selecting on a coin flip.

Pipeline - each scan is decoded ONCE, then resampled to two grids:

    NIfTI -> brain mask, striatal centre, reference level   (computed once)
          -> 64^3 @2.0mm    -> 20 SBR features -> 15 LightGBM boosters     w 0.20
...
Grouped CV (5 folds x 3 seeds, n=1362): log loss 0.2448, AUROC 0.9613.
Public leaderboard: log loss 0.2665.
...
Environment:
  ...
  * assets/            config.json, models/lgb_*.txt, cnn/<family>/*.pt - built by
                       scripts/pack.sh 011; weights are not in the repository
...
from datpark.features import feature_matrix  # noqa: E402
...
CUBE_SCALE = 6.0  # must match SCALE in scripts/train_cnn.py
CNN_BATCH = 16
...
def _all_grids(uid: str):
    try:
        ...
        out["feats"] = feature_matrix(out["g64"][None], voxel_mm=base.voxel_mm)[0]
...
def main() -> None:
    ...
    with ProcessPoolExecutor(max_workers=workers, initializer=_init_worker,
        ...
        try:
            ...
            logger.warning(f"preprocessing pool ended early ({type(exc).__name__}); "
                           f"{sum(r is None for r in results)} scans will use the fallback")
    ...
    if not ok.all():
        logger.warning("At least one scan could not be processed; fallback used.")
