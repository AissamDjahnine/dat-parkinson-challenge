"""Inference entrypoint for the DaT Parkinson's Challenge — model 008a.
...
Pipeline — each scan is decoded ONCE, then resampled to two grids:
    ...
    NIfTI -> brain mask, striatal centre, reference level   (computed once)
          ...
          -> 96^3 @1.333mm  -> sm0      DatNet stem3        x15, TTA si_rl        \\
...
Grouped CV (5 folds x 3 seeds, n=1362): log loss 0.2472, AUROC 0.9607.
Public leaderboard: log loss 0.2653, AUROC 0.9528.
...
Environment:
  ...
  * assets/            config.json, models/lgb_*.txt, cnn/<family>/*.pt — built by
                       scripts/pack.sh 008a; weights are not in the repository
