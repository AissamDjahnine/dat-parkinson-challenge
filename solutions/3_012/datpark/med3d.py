"""3D ResNet encoders with MedicalNet (Med3D) pretrained weights, in pure PyTorch.
    ...
    model = build_med3d(depth=10, weights="<path to the pretrained r10 state_dict>")

WHY THIS EXISTS. The training recipe is exhausted: diversity, hyperparameters and more data per model
were each swept without further gain, and the only remaining route to a better blend is a
