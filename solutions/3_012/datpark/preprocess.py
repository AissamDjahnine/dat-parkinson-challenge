...
def load_volume(path: str | Path) -> tuple[np.ndarray, np.ndarray]:
    ...
    # ORIENTATION. Every downstream step assumes axis 0 = R(+), axis 1 = A(+), axis 2 = S(+):
    # `features_v2` divides left from right on axis 0, and `striatum_centre` searches in that frame. A
    # volume stored in any other orientation would have its left and right silently transposed, which is
    # a total-loss error on a laterality-sensitive measurement.
    #
    # `as_closest_canonical` reorders and flips axes to the closest RAS orientation using the affine, and
    # is an exact identity when the volume is already RAS. It therefore cannot alter a prediction on
    # already-canonical data while removing the failure mode on data that is not.
    #
    # Placed BEFORE get_zooms() deliberately: a reorientation permutes the axes, so voxel sizes must be
    # read from the canonicalised header or they would be paired with the wrong axes.
    try:
        img = nib.as_closest_canonical(img)
    except Exception:  # noqa: BLE001 - a malformed affine must degrade, never abort a submission
        pass
